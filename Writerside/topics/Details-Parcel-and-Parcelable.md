...
## Parcel과 Parcelable {#parcel-and-parcelable}

### Parcel과 Parcelable의 개념과 역할 {#concept-and-role}

`Parcel`과 `Parcelable`은 안드로이드의 고성능 프로세스 간 통신(IPC)을 위해 설계된 핵심 요소이다.

### Parcel: 고성능 IPC를 위한 데이터 컨테이너 {#parcel-container}
...
### Parcelable: 객체의 직렬화 규약 {#parcelable-contract}
...
### 주요 특징 및 제약사항 {#features-and-constraints}
...
* **비영구성**: `Parcel`의 내부 구현은 안드로이드 버전에 따라 변경될 수 있다. 따라서 영구 저장소(DB, 파일 등)에 데이터를 저장하는 용도로 `Parcel`을 사용해서는 안 된다. 오직 **임시적인 데이터 전송 용도**로만 사용해야 한다.
