...
Context는 잘못 사용하면 메모리 누수, 크래시 또는 비효율적인 리소스 처리와 같은 문제가 발생할 수 있다. 
...
컨텍스트가 필요한 수명이 긴 object에는 Application Context를 사용해야 한다.
...
파괴된 컴포넌트에 연결된 컨텍스트에 액세스하면 해당 컨텍스트에 연결된 리소스가 더 이상 존재하지 않을 수 있으므로 크래시나 정의되지 않은 동작이 발생할 수 있다. 
...
백그라운드 스레드에서 사용하면 예기치 않은 크래시나 스레딩 문제가 발생할 수 있다. 
UI 관련 컨텍스트 리소스와 상호 작용하기 전에 메인 스레드로 다시 전환해야 한다.
...
3. 만약 사용자가 네트워크 요청 후 2초가 지나기 전에 화면을 전환하면 LeakActivity는 파괴된다.
...
- 파괴된 컴포넌트의 Context는 사용하지 않기: 컴포넌트가 파괴된 후에는 관련 리소스가 더 이상 유효하지 않을 수 있다. 파괴된 Context에 접근하면 크래시를 유발할 수 있다. 
