...
| IO | 블로킹 I/O | 기본 64개(코어 수가 더 많으면 코어 수) | 네트워크, 파일, DB |
...
같은 풀을 공유하면서도 특정 작업의 동시 실행 수를 제한하고 싶으면 `limitedParallelism(n)`을 씁니다. 대부분 새 스레드 없이 공용 풀 위에서 **동시 실행 수만 n으로 제한한 뷰(view)** 를 돌려줍니다. 다만 `Dispatchers.IO`의 뷰는 IO 자체가 가진 탄력성(elasticity) 때문에 필요 시 IO의 기본 한도와 무관하게 추가 스레드를 확보할 수 있습니다.
