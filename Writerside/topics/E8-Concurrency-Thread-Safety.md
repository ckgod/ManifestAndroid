...
CAS는 락을 잡지 않으므로 락 경합이 적을 때 `synchronized`보다 빠르고, 블로킹이 없어 deadlock도 발생하지 않습니다. 다만 단일 변수의 원자 연산에만 적합하며, **여러 변수를 함께 일관되게 갱신해야 한다면 락(또는 Mutex)이 필요**합니다.
...
- `Atomic`: CAS 연산 자체가 volatile 읽기·쓰기 의미를 포함합니다.
...
- **타임아웃**: `tryLock(timeout)`처럼 일정 시간 안에 못 잡으면 포기하고 잡은 락을 풀어, 점유와 대기를 깹니다.
