...
## 시작하기 전에: null과 null 참조 {#null-basics}

**`null`은 "값이 없음"을 나타내는 특별한 값**입니다. 숫자 `0`이나 빈 문자열 `""`과는 다릅니다. `0`은 "0이라는 값이 있는" 상태지만, `null`은 "가리킬 대상 자체가 없는" 상태입니다.

이를 이해하려면 **참조(reference)** 를 알아야 합니다. 객체 타입의 변수는 객체를 직접 담지 않고, 메모리 어딘가에 있는 객체의 **주소(위치)를 가리키는 참조**를 담습니다. 변수는 실제 객체가 놓인 사물함을 가리키는 열쇠라고 생각하면 됩니다.
...
그래서 **"null을 참조한다"** 는 것은, 가리키는 대상이 없는 이 참조를 통해 객체의 멤버(프로퍼티·메서드)에 접근하려는 시도를 뜻합니다. `null.length`처럼요. 존재하지 않는 사물함을 열어 내용물을 꺼내려는 것과 같아서, 실행하면 오류가 납니다. 이 오류가 바로 아래에서 이야기할 `NullPointerException`입니다.
