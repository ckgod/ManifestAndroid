...
> **먼저 읽으면 좋아요** — `null`이 정확히 무엇인지, "null을 참조한다"는 게 어떤 의미인지, 그리고 `Int`·`Boolean` 같은 원시 타입은 이 그림에 어떻게 들어맞는지는 [Details) null과 참조, 그리고 원시 타입](Details-null과-참조.md)에서 다룹니다. 이 개념이 익숙하지 않다면 먼저 보고 오는 것을 권합니다.
