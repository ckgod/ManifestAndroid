...
### "클래스 본문 실행 전"이라는 표현 {#before-body}

`init`을 두고 "주 생성자 인자가 전달된 후 **클래스 본문이 실행되기 전에** 호출된다"고 설명하는 자료가 많습니다. 이 표현이 오해를 만듭니다.

`init` 블록은 클래스 본문보다 **앞서** 실행되는 것이 아닙니다. `init` 블록 자체가 클래스 본문의 일부이고, 같은 본문에 있는 프로퍼티 초기화자와 나란히 순서를 다툽니다. 본문이 통째로 나중에 실행된다면 위 예제에서 `prop a`와 `prop b`가 먼저 찍히고 `init 1`, `init 2`가 뒤따라야 하는데 그렇지 않습니다.

Kotlin 공식 문서도 초기화 블록이 프로퍼티 초기화자와 **`interleaved`**(끼워 넣어진다)라고 표현합니다. "본문 실행 전"이 아니라 "본문 안에서, 적힌 순서대로"가 정확합니다.

정리하면 순서는 이렇습니다.

1. 주 생성자 괄호 안 프로퍼티 대입
2. 클래스 본문의 프로퍼티 초기화자와 `init` 블록 — **선언 순서대로 섞여서**
3. (보조 생성자를 썼다면) 보조 생성자 본문
