...
즉 **코틀린에서 SAM 변환을 받으려면 `fun interface`로 선언해야** 합니다(뒤 [fun 키워드를 요구하는 이유](#why-fun) 참고). 추상 메서드는 하나여야 하지만, 그 외의 비추상(기본 구현) 멤버는 얼마든 가질 수 있습니다.
...
코틀린은 "더 나은 Java", 즉 이 방대한 Java 라이브러리 생태계와 자연스럽게 맞물리는 것을 목표로 했습니다. 그래서 **Java의 SAM 인터페이스를 기대하는 자리에는 람다를 그냥 넘길 수 있게** 했고, 바이트코드는 컴파일러가 익명 클래스로 만들어 줍니다.
...
즉 함수형 언어와 레거시 객체지향 API 사이의 임피던스 불일치를 개발자 대신 컴파일러가 떠안는 것입니다. 코드는 관용적인 코틀린으로 유지되면서 바이트코드는 Java와 완벽히 호환됩니다.
...
- **추가 멤버가 필요할 때** — `fun interface`는 추상 메서드 하나 외에 기본 구현 메서드나 프로퍼티, `companion object`를 가질 수 있습니다. 함수 타입은 그럴 수 없습니다.
...
2. **SAM 변환**: 람다를 그 인터페이스의 구현 객체로 자동 변환. 익명 객체를 손으로 안 써도 된다. 어느 타입이 될지는 받는 쪽이 결정한다.
3. **`fun interface`**: 코틀린에서 SAM 변환을 받으려면 필요한 선언. 추상 메서드는 하나, 비추상 멤버는 여럿 가능.
4. **Java 상호운용**: `Runnable`·`Comparator`·안드로이드 리스너 등 Java SAM 인터페이스에 람다를 그냥 넘길 수 있다. 이게 SAM 변환의 본래 목적.
...
람다를 함수형 인터페이스의 구현 객체로 컴파일러가 자동 변환하는 것입니다. 변환이 없다면 `object : Runnable { override fun run() { ... } }`처럼 익명 객체를 직접 써야 하지만, SAM 변환 덕분에 `{ ... }` 람다만 넘기면 됩니다. 문법은 Q18의 일반 람다와 똑같고, 넘기는 자리가 함수형 인터페이스를 기대하면 `FunctionN` 대신 그 인터페이스의 구현체로 컴파일됩니다.
