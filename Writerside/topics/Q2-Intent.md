...
- **정의**: 명시적 인텐트는 호출할 정확한 Component(Activity, Service..)를 클래스 이름으로 직접 명시한다.
...
### Intent Filter란?
인텐트 필터는 앱 컴포넌트가 링크 열기 또는 Broadcast 처리와 같은 특정 인텐트에 어떻게 응답할지를 정의한다.
즉, 컴포넌트가 처리할 수 있는 `action`, `category`, `data`의 조합을 manifest에 선언해 두는 것이다.

가장 대표적인 예시는 다른 앱의 `공유하기` 메뉴에 자신의 앱을 노출시키는 것이다.
...
#### 사용 예시
다른 앱에서 텍스트를 공유할 때, 내 앱의 `ShareActivity`가 선택지에 나타나도록 만드는 간단한 예제이다.
...
핵심 차이는 **대상 컴포넌트를 직접 지정하는지 여부**입니다.

명시적 인텐트는 `Intent(this, TargetActivity::class.java)`처럼 호출할 컴포넌트를 클래스 이름으로 직접 명시합니다. 어떤 컴포넌트가 실행될지 컴파일 시점에 확정되므로, 대상이 명확한 **앱 내부 화면 전환**에 사용합니다. 예를 들어 목록 화면에서 상세 화면으로 이동하는 경우입니다.

암시적 인텐트는 대상을 지정하지 않고 `action`, `category`, `data`로 "무엇을 하고 싶은지"만 선언합니다. 실제로 어떤 컴포넌트가 처리할지는 시스템이 런타임에 결정합니다. 따라서 URL 열기, 콘텐츠 공유, 사진 촬영처럼 **다른 앱이나 시스템이 처리해 줄 작업**에 사용합니다.

정리하면, 대상을 알고 있고 내가 직접 호출할 때는 명시적 인텐트를, 작업만 정의하고 처리 주체를 시스템에 위임할 때는 암시적 인텐트를 사용합니다.

</def>
<def title="Q) 안드로이드 시스템은 암시적 인텐트를 처리할 앱을 어떻게 결정하고, 적합한 앱이 없을 경우 어떻게 되나요?">

암시적 인텐트가 전달되면 시스템은 **인텐트 해석(intent resolution)** 과정을 수행합니다. 인텐트의 `action`, `category`, `data`(MIME 타입, URI scheme 등)를 설치된 앱들이 manifest에 선언한 `<intent-filter>`와 대조해, 세 조건을 모두 만족하는 컴포넌트를 후보로 추립니다.

후보 수에 따라 동작이 달라집니다.

- **후보가 하나**: 해당 컴포넌트를 바로 실행하고 인텐트 객체를 전달합니다.
- **후보가 여럿**: 사용자에게 선택 대화 상자(chooser dialog)를 띄워 어떤 앱으로 처리할지 직접 고르게 합니다.

적합한 앱이 하나도 없으면 `startActivity()`는 **`ActivityNotFoundException`** 을 던집니다. 따라서 외부 처리를 기대하는 암시적 인텐트는 `intent.resolveActivity(packageManager) != null` 로 처리 가능한 앱이 있는지 먼저 확인하거나, `try-catch`로 예외를 방어하는 것이 안전합니다.
