...
| 제약 | 내용 |
|---|---|
| 프로퍼티 개수 | 주 생성자에 **읽기 전용 프로퍼티(`val`) 하나만** |
| 상속 | 지원하지 않음(다른 클래스를 상속할 수도, 상속될 수도 없음) |
| nullable | 명시적으로 허용된 경우(`Int?` 등)가 아니면 기본 프로퍼티로 사용 불가 |
| identity | 참조 동등성(`===`)이 제거됨 |

마지막 항목이 특히 중요합니다. 래퍼가 런타임에 사라질 수 있다는 것은 **"이 객체"라고 부를 대상이 없을 수도 있다**는 뜻입니다. 그래서 value class에는 identity 개념이 성립하지 않고, `===` 비교가 컴파일 오류로 막힙니다. 컴파일러가 표현을 자유롭게 최적화할 수 있는 것도 이 제약 덕분입니다.
...
```kotlin
val id1: Any = UserId("abc")                    // Any로 저장 → 박싱 강제
val id2: UserId? = UserId("def")                // nullable → 박싱될 수 있음
...
1. **정의**: 주 생성자에 `val` 하나. 상속 불가, `===` 사용 불가(identity 없음).
...
주 생성자에 읽기 전용 프로퍼티(`val`)를 **단 하나만** 가질 수 있고, 상속을 지원하지 않으며, 명시적으로 허용된 경우가 아니면 기본 프로퍼티로 nullable 타입을 쓸 수 없습니다. 또한 참조 동등성(`===`)이 제거되어 사용 시 컴파일 오류가 납니다. 래퍼가 런타임에 사라질 수 있으므로 "이 객체"라고 가리킬 identity 자체가 성립하지 않기 때문이며, 이 제약 덕분에 컴파일러가 표현을 자유롭게 최적화할 수 있습니다.
