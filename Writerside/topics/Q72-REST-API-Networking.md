...
REST의 또 다른 핵심은 **무상태성**입니다. 서버는 요청 사이에 클라이언트의 상태를 저장하지 않으며, **각 요청은 그 요청을 처리하는 데 필요한 모든 정보를 스스로 담고 있어야** 합니다. 그래서 인증 정보(토큰)를 매 요청 헤더에 실어 보내야 합니다. 서버가 "아까 그 사람"을 기억하지 않기 때문입니다. 이 성질 덕분에 서버를 여러 대로 수평 확장하기 쉽습니다.
...
- Interceptor는 OkHttp의 백그라운드 스레드에서 동작하므로 `Thread.sleep`이 메인 스레드를 막지 않지만, 코루틴 환경이라면 호출부에서 `retryWhen`/`retry` 같은 코루틴 친화적 재시도를 쓰는 선택지도 있습니다.
