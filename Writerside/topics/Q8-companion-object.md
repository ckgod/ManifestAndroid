...
Kotlin에는 `static` 키워드가 없습니다. 그 빈자리를 `companion object`가 채웁니다. 그러니 대체 대상인 `static`이 무엇이었는지부터 짚고 시작하겠습니다.

## Java의 static 되짚기 {#java-static}
...
Kotlin은 이 역할을 `companion object`에 넘겼습니다. 다만 그대로 옮긴 것이 아니라, 접근 방식 자체를 바꿨습니다.
